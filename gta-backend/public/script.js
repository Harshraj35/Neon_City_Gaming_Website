    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if(!target && link.classList.contains('play-now-btn')) {
                ...
                target = localStorage.getItem('gta_token') ? 'dashboard-page' : 'login-page';
            }
            if (target) {
                e.preventDefault();
                navigateTo(target);
        ...
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    errorDiv.textContent = data.message || (data.errors && data.errors[0].msg) || 'Registration failed.';
                    errorDiv.style.display = 'block';
                }
            } catch (err) {
                errorDiv.textContent = 'Network Error. Could not register.';
                errorDiv.style.display = 'block';
            }
        });
    }
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.ok) {
                    // Success
                    localStorage.setItem('gta_token', data.token);
                    localStorage.setItem('gta_user', JSON.stringify({ username: data.username, level: data.level, coins: data.coins, rank: data.rank }));
                    ...
                    errorDiv.textContent = data.message || 'Invalid security clearance!';
                    errorDiv.style.display = 'block';
                }
            } catch (err) {
                errorDiv.textContent = 'Network Error. Could not login.';
                errorDiv.style.display = 'block';
            }
        });
    }
